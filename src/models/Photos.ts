...
export const savePhoto = async (files: fileUpload.UploadedFile | fileUpload.UploadedFile[]): Promise<IPhoto[] | undefined> => {
  try {
    ...
    for (const uploadedFile of uploadedFiles) {
      const sql = `INSERT INTO gallery (public_id, url) VALUES ('${uploadedFile.public_id}', '${uploadedFile.url}')`;
...
export const getPhotos = async (): Promise<IPhoto[] | undefined> => {
  try {
    const sql = "SELECT public_id, url FROM gallery";
